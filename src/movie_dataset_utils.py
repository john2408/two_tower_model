...
def load_data(path: str = "./data/") -> tuple:
    """Load data from csv files and return as numpy arrays
    for the subset of the movie dataset prepared
    ...
    item_train_path = os.path.join(path, "content_item_train.csv")
    user_train_path = os.path.join(path, "content_user_train.csv")
    y_train_path = os.path.join(path, "content_y_train.csv")
    
    item_train = genfromtxt(item_train_path, delimiter=',')
    user_train = genfromtxt(user_train_path, delimiter=',')
    y_train    = genfromtxt(y_train_path, delimiter=',')
    
    item_train_header_path = os.path.join(path, "content_item_train_header.txt")
    with open(item_train_header_path, newline='') as f:    #csv reader handles quoted strings better
        item_features = list(csv.reader(f))[0]
        
    user_train_header_path = os.path.join(path, "content_user_train_header.txt")
    with open(user_train_header_path, newline='') as f:
        user_features = list(csv.reader(f))[0]
        
    items_vecs_path = os.path.join(path, "content_item_vecs.csv")
    item_vecs = genfromtxt(items_vecs_path, delimiter=',')
       
    movie_dict = defaultdict(dict)
    count = 0

    content_movie_list_path = os.path.join(path, "content_movie_list.csv")
    with open(content_movie_list_path, newline='') as csvfile:
    ...
    content_user_to_genre_path = os.path.join(path, "content_user_to_genre.pickle")
    with open(content_user_to_genre_path, 'rb') as f:
